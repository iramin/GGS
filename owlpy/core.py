...
def segmentTimeSeries(MPIndex):
    print("segmentTimeSeries")
    profileLen = len(MPIndex)
    nnmark = np.zeros(profileLen + 1)
    print(len(nnmark))
    for i in range(0, profileLen):
        small = int(min(i, MPIndex[i]))
        large = int(max(i, MPIndex[i]))
        nnmark[small] = nnmark[small] + 1
        nnmark[large] = nnmark[large] - 1

    return np.cumsum(nnmark)

def normCrossCountAll(crosscount,slWindow):
    print("normCrossCountAll")
    l = len(crosscount)
    for i in range(1, l):
        ac = crosscount[i]
        ic = 2 * i * (l-i) / l
    ...
    for i in range(0, zone):
        crosscount[i] = 1
    for j in range(l-zone,l):
        crosscount[j] = 1

    return crosscount


def runSegmentation(ts, slWindow ):
    print("runSegmentation")
    MP, MPIndex = stamp(ts,ts,slWindow)
    print(MPIndex)
    print(min(MPIndex))
    print(max(MPIndex))
    print(len(MPIndex))
    crosscount = segmentTimeSeries(MPIndex)
    return normCrossCountAll(crosscount, slWindow)
